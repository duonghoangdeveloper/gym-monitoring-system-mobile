...
import { CommonScreenLoading } from './common-screen-loading';
...
export const AppApolloWrapper = ({ children }) => {
    ...
    const requestLink = new ApolloLink(
      (operation, forward) =>
        new Observable(observer => {
          let handle;
          Promise.all([Promise.resolve(operation), getTokenFromSecureStorage()])
            .then(([oper, token]) => {
              oper.setContext({
                headers: {
  ...
  if (!client) {
    return <CommonScreenLoading fullscreen />;
  }
